...
| **设置中心** | 全部配置的图形界面，改动即时生效并写入 `config.toml` |
...
| 原生渲染 + 首次打开过 Flyout 或设置窗口 | 7 | 约 230–290 MB | — |

最后一行需要如实说明：**Flyout 和设置中心仍然是 WebView2**。

- **设置中心**：按需创建，**关闭窗口即销毁**，所以只在设置打开期间占用。
- **Flyout**：按需创建，但关闭后只隐藏（重建一个 webview 要几百毫秒，点击时很卡），
  所以**第一次打开 Flyout 之后，Chromium 进程树会一直留到退出**。
...
**设置中心仍然依赖 WebView2。**
打开设置会临时拉起一整套 Chromium 进程（实测约 190 MB），关闭窗口即释放。这是有意的
取舍：设置页是表单、下拉、滑块的密集组合，控件工具箱在这里真正省事，而它并不常开。

**原生小组件栏没有无障碍树。**
它是一块 Direct2D 画布，屏幕阅读器看不到启动器按钮和播放控件——原来的 WebView2 版本
是有无障碍树的。需要无障碍支持的话，在设置里切回 WebView2 渲染。
